...
class Ship {
	...
	updateVelocity(keys) {
		if (keys.up) {
			this.velocity.x += Math.cos(degreesToRadians(this.direction)) * this.acceleration;
			this.velocity.y += Math.sin(degreesToRadians(this.direction)) * this.acceleration;
		}
	...
	shoot() {
		...
		const canShoot = Date.now() - this.lastBullet > fireInterval || !this.lastBullet;
	...
	rocketThrust() {
		...
		const particleDirection = this.direction + 180 >= 360 ? this.direction + 180 : this.direction - 180;
		const particleVelocity = calculateVector(getRandomNum(3, 5), particleDirection);
